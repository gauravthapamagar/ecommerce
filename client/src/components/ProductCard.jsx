    ...
    return product && (
        <div onClick={() => {navigate(`/products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0);}} className="border border-gray-500/20 rounded-md md:px-4 px-3 py-2 bg-white w-full">
            <div className="group cursor-pointer flex items-center justify-center px-2">
                <img className="group-hover:scale-105 transition w-full h-auto max-h-48 max-w-26 md:max-w-36" src={product?.image?.[0]} alt={product.name} />
            </div>
                ...
                <div className="flex items-end justify-between mt-3">
                    ...
                    <div className="text-primary">
                        {!cartItems[product._id] ? (
                            <button onClick={(e) => {
                                e.stopPropagation(); // ⛔ Stop click from bubbling to parent div (which triggers navigation)
                            ...
                            <div onClick={(e) => e.stopPropagation()} className="flex items-center justify-center gap-2 md:w-20 w-16 h-[34px] bg-primary/25 rounded select-none">
